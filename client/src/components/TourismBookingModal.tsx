...
//       setMobileNumber("")
//       setTravelType("")
//       setTravellers([{ name: "", age: 0, gender: "" }])
//       setStep("FORM")
...
//     if (!travelType) {
//       toast.error("Please select a trip style")
//       return
//     }
...
//       if (!t.name.trim()) {
//         toast.error(`Please enter a name for Traveller ${i + 1}`)
//         return
//       }
//       if (t.age <= 0 || isNaN(t.age)) {
//         toast.error(`Please enter a valid age (> 0) for Traveller ${i + 1}`)
//         return
//       }
//       if (!t.gender) {
//         toast.error(`Please select a gender for Traveller ${i + 1}`)
//         return
//       }
//     }
...
//           toast.error(
//             orderRes.data?.error || "Failed to initialize booking order"
//           )
...
//             try {
//               const verifyRes = await _axios.post(
//                 "/booking/standard/verify-payment",
//                 {
...
//             }}
//           >
//             📍 {pkg.destination} &nbsp;•&nbsp; 🗓 {pkg.days} Days / {pkg.nights}{" "}
//             Nights
//           </div>
//         </div>
...
//                     >
//                       {new Date(
//                         successBooking.travellerInfo?.travelDate
...
//                     }}
//                   >
//                     <strong>Customized Tour:</strong> Complete your details
//                     below. No upfront payment required—our advisors will share a
//                     custom quote.
//                   </div>
//                 </div>
//               )}
...
//                         value={t.gender}
//                         onChange={(e) =>
//                           handleTravellerChange(idx, "gender", e.target.value)
//                         }
//                         style={{
//                           padding: "6px 8px",
//                           border: "1px solid #E8E4DC",
...
//                           cursor: "pointer",
//                           opacity:
//                             validatingCoupon || !couponCodeInput.trim()
//                               ? 0.6
//                               : 1,
//                         }}
//                       >
...
const emptyTraveller = (): TravellerRow => ({ name: "", age: "", gender: "" })
...
export const TourismBookingModal: React.FC<TourismBookingModalProps> = ({
  ...
  // Populate user defaults — ONLY full name & email are prefilled.
  // Everything else (mobile, trip style, traveller details) starts blank
  // and must be typed/selected by the user.
  useEffect(() => {
    if (isOpen && pkg) {
      if (user) {
  ...
  const handleSubmitBooking = async (e: React.FormEvent) => {
    e.preventDefault()
    console.log("err00")
    // --- Contact details validation (field-specific messages) ---
    if (!fullName.trim()) {
      console.log("err00")
    ...
    // --- Traveller list validation (field-specific, per traveller) ---
    for (let i = 0; i < travellers.length; i++) {
      const t = travellers[i]
      if (!t.name.trim()) {
        console.log("err0011")
