...
// ── Panel Background styles (Tailwind can't do arbitrary radial-gradients) ──
const panelBgStyles: Record<string, React.CSSProperties> = {
  "bg-cardiac": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #c8d8f8, #a0b8ef 50%, #7898e0 100%)",
  },
  "bg-ortho": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #b8e8d8, #80c8b0 50%, #50a890 100%)",
  },
  "bg-cosmetic": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #f8d0dc, #e8a0b8 50%, #d07898 100%)",
  },
  "bg-oncology": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #d8c8f8, #b098e0 50%, #8868c8 100%)",
  },
  "bg-ivf": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #f8e8b0, #e8c868 50%, #c8a030 100%)",
  },
  "bg-dental": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #b8e8f8, #70c8f0 50%, #30a8d8 100%)",
  },
  "bg-hair": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #d0f0c0, #a0d880 50%, #70b850 100%)",
  },
  "bg-neuro": {
    background:
      "radial-gradient(ellipse 80% 80% at 70% 30%, #e0d0f8, #c0a0e8 50%, #9870d0 100%)",
  },
}
...
// ── Panel Wrapper (bg + overlay + content) ─────────────
const Panel = ({
  s,
  active,
  isMobile = false,
  ...
  >
    {/* BG */}
    <div className="absolute inset-0" style={panelBgStyles[s.bgClass]} />
    {/* Overlay */}
    <div
      className="absolute inset-0"
      style={{
        background:
          "linear-gradient(115deg, rgba(238,241,248,0.97) 0%, rgba(238,241,248,0.88) 50%, rgba(238,241,248,0.25) 100%)",
      }}
    />
    {/* Blob */}
    <div
      className="pointer-events-none absolute -top-16 -right-16 h-80 w-80 rounded-full opacity-[0.18]"
      style={panelBgStyles[s.bgClass]}
    />
    <PanelContent s={s} isMobile={isMobile} />
  </div>
)
...
const Services: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-[1280px] overflow-x-hidden rounded-xl px-5 py-14 text-[#1a1a2e] md:px-8 lg:px-10">
      ...
      {/* <div
        ref={headerRef}
        className="px-5 pt-7 pb-4 transition-all duration-700"
        style={{
          opacity: headerVisible ? 1 : 0,
          transform: headerVisible ? "none" : "translateY(28px)",
        }}
      >
        <div className="mb-2 flex items-center gap-2.5 font-mono text-[10px] font-bold tracking-[0.28em] text-green-700 uppercase">
          <span className="h-px w-5 flex-shrink-0 bg-green-700" />
          Our Specialties
        </div>
        <h1 className="font-serif text-[26px] leading-[1.15] font-bold tracking-[-0.02em] text-[#1B2B6B]">
          World-Class Care,
          <br />
          <em className="text-green-700 not-italic">Every Specialty</em>
        </h1>
      </div> */}
